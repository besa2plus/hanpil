...
3. **저장소 준비**  
   ...
   - Module 개발에 필요한 언어별 패키지 매니저를 준비합니다 (자세한 내용은 아래 "Module 개발 환경" 참조).
...
5. **Module PR 체크리스트**  
   - 파일이 500줄 이하인지 확인
   - `metadata`에 `name`, `description`, `version`, `author`, `contributors`, `language`, `package_manager`, `isolation`이 모두 포함되어 있는지 확인
   - `dependencies`가 필요하면 최소화하고, 순환 의존이 없는지 확인
   - copy로 가져온 코드가 있다면 원본 작성자를 `contributors`에 추가
   - AI/ML, 네이티브 확장 등 격리가 필요한 경우 `isolation`을 `"venv"` 또는 `"local"`로 설정했는지 확인
...
- `language`: Module 구현 언어 (`typescript`, `python`, `rust` 등)
- `package_manager?`: 사용 패키지 매니저 (`pnpm`, `uv`, `cargo`, `gradle` 등)
- `install_command?`: 설치 명령어 (예: `"uv sync"`)
- `runtime_requirements?`: 런타임 요구사항 (예: `["python>=3.11"]`)
- `isolation?`: 환경 격리 정책 (`"none"` | `"venv"` | `"local"`)
  - `"none"` (기본): 프로젝트 공통 환경 공유
  - `"venv"` / `"local"`: Module 내부에 독립 가상환경 또는 로컬 의존성 사용 (AI/ML, 네이티브 확장 등에 권장)

### Module 개발 환경

hanpil은 EMP 아키텍처를 따르며, Module은 다양한 언어로 작성될 수 있습니다.  
각 Module은 **해당 언어의 표준 패키지 매니저**를 사용합니다.

- TypeScript/JavaScript Module → `pnpm` (권장) 또는 `npm`
- Python Module → `uv` (권장) 또는 `pip` / `poetry`
- Rust Module → `cargo`
- Kotlin/Java Module → `gradle` / `maven`
- Go Module → `go mod`

Module을 만들 때는 해당 언어의 관례를 따르고, `metadata`에 `language`, `package_manager`, `install_command` 등을 반드시 기입해주세요.  
Engine은 Registry 정보를 바탕으로 Module 설치 시 적절한 명령어를 실행합니다.

### Module 환경 격리 정책

Module은 기본적으로 프로젝트 공통 환경을 공유하는 것을 권장합니다.  
AI/ML 모델, 네이티브 확장, breaking change 위험이 큰 의존성 등 충돌 가능성이 높은 경우에만 격리를 사용하세요.

- **기본 (isolation 생략 또는 `"none"`)**: `runtime_requirements`를 가능한 한 넓은 범위로 작성하여 여러 Module이 Python/Node 버전을 공유합니다. 예: `"python>=3.11"`, `"node>=20"`
- **격리 필요 (`"venv"` 또는 `"local"`)**: AI/ML 라이브러리(torch, tensorflow 등), 네이티브 확장 모듈, 특정 버전에 강하게 의존하는 경우 Module 내부에 독립 환경을 구성합니다.
- **install_command 작성 시**: 항상 Module 디렉토리 기준으로 작성하여 루트 환경을 오염시키지 마세요. Engine은 Module 작업 디렉토리를 정확히 지정하여 실행합니다.

충돌이 발생하면 Engine은 자동 해결을 시도하지 않고, 해당 Module의 metadata를 확인하도록 안내합니다.
